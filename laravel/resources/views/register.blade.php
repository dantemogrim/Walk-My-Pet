...
<h3 class="mr-3 my-3 font-bold text-3xl">Register:</h3>
<form class="p-2 bg-white shadow rounded" action="register" method="post">
    @csrf
    <div>
        <label class="font-semibold mr-1" for="name">Name:</label>
        <input class="my-2 bg-white-100 border-green-500 border-2 rounded-sm" name="name" id="name" type="name" placeholder="What is your name?" />
    </div>
    <div>
        <label class="font-semibold mr-1" for="email">E-mail:</label>
        <input class="my-2 bg-white-100 border-green-500 border-2 rounded-sm" name="email" id="email" type="email" placeholder="Enter your email..." required/>
    </div>
    <div>
        <label class="font-semibold mr-1" for="password">Password (at least 8 characters required):</label>
        <input class="my-2 bg-white-100 border-green-500 border-2 rounded-sm" name="password" id="password" type="password" placeholder="Choose your password..." required/>
    </div>
    <div>
        <label class="font-semibold mr-1" for="password-verify">Confirm password:</label>
        <input class="my-2 bg-white-100 border-green-500 border-2 rounded-sm" name="password-verify" id="password-verify" type="password" placeholder="Repeat..." required/>
    </div>
    <button class="ml-2 my-3 py-2 px-4 bg-green-500 text-white font-semibold rounded-lg shadow-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-opacity-75" type="submit">Register</button>
</form>
</div>
